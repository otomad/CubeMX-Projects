...
//-----------------------------------------------------------------
//
// 函数功能: SRAM底层驱动，时钟使能，引脚分配
// 入口参数: SRAM_HandleTypeDef *hsram：SRAM句柄
// 返回参数: 无
// 注意事项: 此函数会被HAL_SRAM_Init()调用
//
...
void HAL_SRAM_MspInit(SRAM_HandleTypeDef *hsram)
{
  GPIO_InitTypeDef GPIO_Initure;
	...
	GPIO_Initure.Pin=GPIO_PIN_8;          // PB8,背光控制
	GPIO_Initure.Mode=GPIO_MODE_OUTPUT_PP;// 推挽输出
	GPIO_Initure.Pull=GPIO_NOPULL;        // 无上拉下拉
	GPIO_Initure.Speed=GPIO_SPEED_HIGH;   // 高速
	HAL_GPIO_Init(GPIOB,&GPIO_Initure);
	
	GPIO_Initure.Pin=GPIO_PIN_4;          // PH4,复位
	GPIO_Initure.Mode=GPIO_MODE_OUTPUT_PP;// 推挽输出
	GPIO_Initure.Pull=GPIO_NOPULL;        // 无上拉下拉
	GPIO_Initure.Speed=GPIO_SPEED_HIGH;   // 高速
...
void LCD_Init(void)
{
	...
	GPIO_Initure.Pin=GPIO_PIN_6;        // PC6 -> RGB_BL
	GPIO_Initure.Mode=GPIO_MODE_INPUT;  // 输入
	GPIO_Initure.Pull=GPIO_PULLDOWN;    // 下拉
	GPIO_Initure.Speed=GPIO_SPEED_HIGH; // 高速
	HAL_GPIO_Init(GPIOC,&GPIO_Initure); // 初始化
	
	GPIO_Initure.Pin=GPIO_PIN_8; 				// PB8	-> LCD_LED
	...
	else
	{
		...
		// FSMC写时序控制寄存器
		FSMC_WriteTim.AddressSetupTime=0x04;            // 地址建立时间(ADDSET)
		FSMC_WriteTim.AddressHoldTime=0x00;
		FSMC_WriteTim.DataSetupTime=0x04;               // 数据保存时间(DATAST)
		...
		if( lcddev.id != ST7789V_ID )                                // 驱动型号非 ST7789V
		{
			...
			if ( lcddev.id != SPFD5408_ID )
			{
				...
				LCD_ReadReg ( 0xC500 ); lcddev.id  = LCD_RD_DATA();      // 读到0x55
				lcddev.id <<= 8;
				LCD_ReadReg ( 0xC501 ); lcddev.id |= LCD_RD_DATA();      // 读取0x10
			}
		}
		if ( lcddev.id == ST7789V_ID )                               // ST7789V
		{
			...
			LCD_WR_REG ( 0X36 );  LCD_WR_DATA ( lcddev.scan );      // 设置扫描方向
	...
	if(lcddev.id == ST7789V_ID || lcddev.id == SPFD5408_ID)
		LCD_POINT = 16;
	else if(lcddev.id == NT35510_ID || lcddev.id == 0x8000 || lcddev.id == 0x7016)
		LCD_POINT = 24;
}
